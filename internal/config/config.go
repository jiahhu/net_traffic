...
func Parse() Config {
	...
	flag.DurationVar(&c.Interval, "interval", envDuration("NETTRAFFIC_INTERVAL", 5*time.Second), "sampling interval")
	...
	flag.BoolVar(&c.ResolveHostnames, "resolve-hostnames", envBool("NETTRAFFIC_RESOLVE_HOSTNAMES", false), "reverse-resolve destination IPs")
	flag.BoolVar(&c.Destinations, "destinations", envBool("NETTRAFFIC_DESTINATIONS_ENABLED", false), "enable destination ranking via conntrack")
